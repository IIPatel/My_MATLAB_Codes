...
C_p=epsS_33 * W * Lbi / Hp;%Effective Capacitance of the Bimorph for Parralel Connection
...
Theta=d_31*(k_eq*W*(Lbi+10e-2)*Hp/2/I);

t = [0:length(w)-1]';
num2 = -2*j*R_L*Theta*m_eq.*w.^3.*exp(j.*w.*t);
den2 = (w_n^2-w.^2+j*2*zeta_m.*w.*w_n).*(m_eq+j*2*m_eq*R_L*C_p.*w)+j*4*R_L*Theta^2.*w;
